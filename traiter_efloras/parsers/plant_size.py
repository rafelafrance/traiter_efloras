...
from traiter_efloras.pylib.convert_units import convert as convert_units
...
from traiter_efloras.pylib.shared_patterns import VOCAB, part_phrase
    ...
    for dim in ('length', 'width'):
        for value in ('min', 'low', 'high', 'max'):
            ...
            if key in token.group:
                norm = convert_units(
                    util.to_float(token.group[key]), units[dim])
...
def parser(plant_part):
    ...
    return Base(
        ...
        rules=[
            ...
            part_phrase(catalog, plant_part),
