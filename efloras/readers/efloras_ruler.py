...
def parse_traits(args, atoms, text):
    ...
    for i, (atom_start, name_end) in enumerate(atoms[:-1]):
        ...
        rulers = {t for t in ar.ALL_RULERS.get(atom_name)
...
def find_atom_names(text):
    """Break text into slices that are used to look for particualr traits."""
    # The sections start with a keyword and extend up to the next keyword
    atoms = [(m.start(), m.end()) for m in ar.RULER_GROUPS_RE.finditer(text)]
    atoms.append((-1, -1))  # Sentinel
    return atoms


def get_treatement(path):
...
def get_traits(treatment):
    ...
    for para in treatment.select('p'):
        ...
        match = ar.RULER_GROUPS_RE.search(text)
        if match:
            return text
    return None
