...
from traiter.tokenizer_util import append_tokenizer_regex
...
MATCHERS = [MARGIN_SHAPE, SIZE]

TERM_MATCHERS = [RANGE]
UPDATES_TERM = [COUNT]  # , SIZE]
ENTITY_MATCHERS = [COLOR, FORGET, SHAPE]
...
def trait_pipeline():
    ...
    term_ruler = nlp.add_pipe(
        'entity_ruler', name='term_ruler', config=config, before='tagger')
    ...
    nlp.add_pipe(ADD_ENTITY_DATA, config={'actions': from_matchers(*ENTITY_MATCHERS)})

    # add_debug_pipes(nlp, 'after entity_data', entities=True)  # ####################

    nlp.add_pipe(DEPENDENCY, name='part_linker', config={'patterns': LINKERS})
